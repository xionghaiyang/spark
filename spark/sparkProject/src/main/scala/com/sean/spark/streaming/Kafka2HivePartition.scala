...
import com.sean.spark.util.{Common, ConfigurationManager, DataSourceUtil, GetTableInfo, MysqlProxy, QueryCallback}
      ...
      if (batchCount > 0) {
        try {
          ...
          tabList.foreach((tab: String) => {
            ...
            val jsonRDD: RDD[String] = name2JsonRDD.filter(kv => {
            ...
            if (!jsonRDD.isEmpty()) {
              ...
              val jsonDataSet: Dataset[String] = spark.createDataset(jsonRDD)
